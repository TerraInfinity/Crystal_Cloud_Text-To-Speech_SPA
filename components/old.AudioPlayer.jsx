...
const AudioPlayer = () => {
  ...
  // Create SpeechServiceAPI instance with fileStorageActions
  ...
  const validSections = sections.filter(
    (section) =>
      ...
      section.text?.trim() &&
      section.voice &&
      section.voice.engine // Ensure voice and engine exist
  );
  ...
  const handleGenerateAudio = () => {
    ...
    generateAllAudio({
      ...
      speechServiceAPI, // Pass the SpeechServiceAPI instance
    })
      .catch((error) => {
  ...
  const handleMergeAudio = async () => {
    ...
    try {
      ...
      if (result && result.mergedAudioUrl) {
        // Update session state with merged audio and title
        ...
        throw new Error('No merged audio URL returned');
      }
    } catch (error) {
