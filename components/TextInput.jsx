  ...
  useEffect(() => {
    if (Object.keys(audioLibrary).length === 0 && !isProcessing) {
      const fetchAudioList = async () => {
        try {
          const files = await ttsActions.listFromStorage('fileStorage');
          ...
          ttsActions.loadAudioLibrary(soundEffects);
        } catch (error) {
          sessionActions.setError('Error fetching audio library: ' + error.message);
        }
      };
      fetchAudioList();
    }
