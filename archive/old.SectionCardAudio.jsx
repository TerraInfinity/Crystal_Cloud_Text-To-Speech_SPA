...
const SectionCardAudio: React.FC<{ section: any }> = ({ section }) => {
  ...
  const normalizeUrl = (url: string, isLibraryAudio: boolean): string => {
    ...
    if (normalizedUrl.includes('://') || normalizedUrl.startsWith('http://') || normalizedUrl.startsWith('https://')) {
      try {
        ...
        return normalizedUrl; // Fallback to original if invalid
      }
    }
    // Explicitly remove incorrect bases
    const incorrectBases = [
      'http://localhost:3000',
      'https://localhost:3000',
      window.location.origin, // Current browser URL
    ];
    incorrectBases.forEach((base) => {
      if (normalizedUrl.startsWith(base)) {
        normalizedUrl = normalizedUrl.replace(base, '');
        devLog(`Removed incorrect base ${base}:`, normalizedUrl);
      }
    });
    // Use /audio/ for library audio, /Uploads/ for uploaded audio
  ...
  const validateAudioUrl = (url: string, source: string | undefined): string => {
    ...
    // For uploaded audio (not yet saved to library), verify blob URL
    if (source === 'upload') {
      if (!url.startsWith('blob:')) {
        devLog('Invalid blob URL for uploaded audio:', url);
        sessionActions.setNotification({
          type: 'error',
          message: 'Invalid uploaded audio URL.',
        });
        return '';
      }
      devLog('Validated upload audio URL:', { url, source });
      return url;
    }
    // For library audio or undefined source, use serverUrl with /audio/
    const normalizedUrl = normalizeUrl(url, true); // true for library audio
    if (!normalizedUrl) {
  ...
  const saveToLibrary = async () => {
    ...
    try {
      ...
      // Normalize audio_url with /Uploads/ for uploaded audio
      const normalizedAudioUrl = normalizeUrl(addedAudio.audio_url, false); // false for uploaded audio
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (audio && audio.audio_url) {
                // Normalize audio_url with /audio/ for library audio
                const normalizedAudioUrl = normalizeUrl(audio.audio_url, true); // true for library audio
          ...
          <>
            {playingAudio ? (
              <audio
                ...
                onError={(e) => {
                  const playbackUrl = validateAudioUrl(audioData.url, audioData.source);
                  devLog('Audio playback error:', e, { playbackUrl, audioData, serverUrl });
                  sessionActions.setNotification({
                    type: 'error',
                    message: 'Failed to play audio. Please check the file path or server configuration.',
                  });
                  setPlayingAudio(false);
                }}
                onLoadStart={() => {
                  const playbackUrl = validateAudioUrl(audioData.url, audioData.source);
                  devLog('Audio loading:', { playbackUrl, source: audioData.source, serverUrl });
                }}
              >
                <source
                  src={validateAudioUrl(audioData.url, audioData.source)}
                  type={audioData.format ? `audio/${audioData.format}` : audioData.type || 'audio/wav'}
                />
