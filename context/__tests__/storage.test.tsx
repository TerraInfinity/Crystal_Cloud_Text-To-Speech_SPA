import {
  ...
  describe('storage', () => {
    ...
    test('saveToStorage uploads file for fileStorage with metadata', async () => {
      mockedAxios.post.mockResolvedValue({ data: { url: '/audio/test.mp3' } });
      const file = new File(['content'], 'test.mp3', { type: 'audio/mpeg' });
      const metadata = { name: 'Test Audio', volume: 0.8 };
      const url = await saveToStorage('test.mp3', file, 'fileStorage', metadata);
  
      expect(mockedAxios.post).toHaveBeenCalledWith(
        'http://localhost:5000/upload',
        expect.any(FormData)
      );
      expect(url).toBe('http://localhost:5000/audio/test.mp3');
    });
    ...
    test('saveToStorage throws error for non-File in fileStorage', async () => {
      await expect(saveToStorage('testKey', 'notAFile', 'fileStorage')).rejects.toThrow(
        'Value must be a File for fileStorage'
      );
    });
    ...
    test('listFromStorage lists localStorage keys', async () => {
      ...
      // Mock localStorage getItem to return from mock data
    ...
    test('listFromStorage lists sessionStorage keys', async () => {
      ...
      // Mock sessionStorage getItem to return from mock data
